...
root.addInode(
  ...
  new Inode({
    file: true,
    executable: true,
    raw: "(function(){\"use strict\";function wrap(style,str){const[open,close]=ansi[style];return`\\x1b[${open}m${str}\\x1b[${close}m`}function colorize(styles,str){if(styles instanceof Array)for(let i in styles)str=wrap(styles[i],str);else\"string\"==typeof styles&&(str=wrap(styles,str));return str}const Ok=x=>({map:f=>Ok(f(x)),chain:f=>f(x),chainOk:f=>f(x),chainErr:()=>Ok(x),fold:()=>pass=>pass(x),toString:()=>`Ok(${x})`}),Err=x=>({map:()=>Err(x),chain:()=>Err(x),chainOk:()=>Err(x),chainErr:f=>f(x),fold:fail=>()=>fail(x),toString:()=>`Err(${x})`}),match=to=>(...cases)=>def=>{for(let i in cases){const c=cases[i];for(let i in c[0])if(c[0][i]==to)return c[1](c[0][i])}return def(to)},type=value=>Object.prototype.toString.call(value).match(/\\[object (.+)\\]/i)[1],serialize=(value,depth=0,maxDepth=5)=>depth>maxDepth?\"[...]\":null==value?\"null\":value==void 0?\"undefined\":\"Object\"==type(value)?(()=>{const ret=Object.keys(value).reduce((acc,key)=>Object.assign({},acc,{[key]:serialize(value[key],depth+1,maxDepth)}),{});return 0==depth?JSON.stringify(ret,null,2):ret})():\"Array\"==type(value)?(()=>{const ret=value.map(x=>serialize(x,depth+1,maxDepth));return 0==depth?JSON.stringify(ret,null,2):ret})():value.inspect?value.inpect():value.toString?value.toString():value.name?value.name:type(value),runSafe=jsCodeString=>{let result;try{result=Ok(self.eval(jsCodeString))}catch(e){result=Err(e)}return result};var evaluate=str=>runSafe(str).map(serialize).fold(e=>cli.colorize(\"red\",e))(r=>cli.colorize(\"green\",r));const esc=\"\\x1B\",beep=\"\\x07\";var control=Object.freeze({cursor:{move:{to:(x=1,y=1)=>esc+\"[\"+x+\";\"+y+\"H\",up:(n=1)=>esc+\"[\"+n+\"A\",down:(n=1)=>esc+\"[\"+n+\"B\",right:(n=1)=>esc+\"[\"+n+\"C\",left:(n=1)=>esc+\"[\"+n+\"D\",nextLine:()=>esc+\"[E\",prevLine:()=>esc+\"[F\",leftMost:()=>esc+\"[G\"},hide:()=>esc+\"[?25l\",show:()=>esc+\"[?25h\",shape:{block:()=>esc+\"]50;CursorShape=0\"+beep,bar:()=>esc+\"]50;CursorShape=1\"+beep,underscore:()=>esc+\"50;CursorShape=2\"+beep},savePosition:()=>esc+\"[s\",restorePosition:()=>esc+\"[u\"},line:{eraseEnd:()=>esc+\"[K\",eraseStart:()=>esc+\"[1K\",erase:()=>esc+\"[2K\"},screen:{eraseDown:()=>esc+\"[J\",eraseUp:()=>esc+\"[1J\",erase:()=>esc+\"[2J\",clear:()=>esc+\"c\",scrollUp:(n=1)=>esc+\"[\"+n+\"S\",scrollDown:(n=1)=>esc+\"[\"+n+\"T\"},misc:{beep:()=>beep,setTitle:str=>esc+\"]0;\"+str+beep,link:(url,str)=>esc+\"[8;;\"+url+beep+(str||url)+esc+\"[8;;\"+beep}}),ansi={reset:[0,0],bold:[1,22],dim:[2,22],italic:[3,23],underline:[4,24],inverse:[7,27],hidden:[8,28],strikethrough:[9,29],black:[30,39],red:[31,39],green:[32,39],yellow:[33,39],blue:[34,39],magenta:[35,39],cyan:[36,39],white:[37,39],gray:[90,39],grey:[90,39],brightRed:[91,39],brightGreen:[92,39],brightYellow:[93,39],brightBlue:[94,39],brightMagenta:[95,39],brightCyan:[96,39],brightWhite:[97,39],bgBlack:[40,49],bgRed:[41,49],bgGreen:[42,49],bgYellow:[43,49],bgBlue:[44,49],bgMagenta:[45,49],bgCyan:[46,49],bgWhite:[47,49],bgGray:[100,49],bgGrey:[100,49],bgBrightRed:[101,49],bgBrightGreen:[102,49],bgBrightYellow:[103,49],bgBrightBlue:[104,49],bgBrightMagenta:[105,49],bgBrightCyan:[106,49],bgBrightWhite:[107,49]};const info=colorize(\"blue\",\"\\u2139\"),success=colorize(\"green\",\"\\u2714\"),warning=colorize(\"yellow\",\"\\u26A0\"),error=colorize(\"red\",\"\\u2716\"),star=colorize(\"brightYellow\",\"\\u2605\"),radioOn=colorize(\"green\",\"\\u25C9\"),radioOff=colorize(\"red\",\"\\u25EF\"),checkboxOn=colorize(\"green\",\"\\u2612\"),checkboxOff=colorize(\"red\",\"\\u2610\");var symbols=Object.freeze({info:info,success:success,warning:warning,error:error,star:star,radioOn:radioOn,radioOff:radioOff,checkboxOn:checkboxOn,checkboxOff:checkboxOff,arrowUp:\"\\u2191\",arrowDown:\"\\u2193\",arrowLeft:\"\\u2190\",arrowRight:\"\\u2192\",line:\"\\u2500\",play:\"\\u25B6\",pointer:\"\\u276F\",pointerSmall:\"\\u203A\",square:\"\\u2587\",squareSmall:\"\\u25FC\",bullet:\"\\u25CF\"});var spinners=Object.freeze({line:{fps:8,frames:[\"-\",\"\\\\\",\"|\",\"/\"]},dots:{fps:12.5,frames:[\"\\u280B\",\"\\u2819\",\"\\u2839\",\"\\u2838\",\"\\u283C\",\"\\u2834\",\"\\u2826\",\"\\u2827\",\"\\u2807\",\"\\u280F\"]},scrolling:{fps:5,frames:[\".  \",\".. \",\"...\",\" ..\",\"  .\",\"   \"]},scrolling2:{fps:2.5,frames:[\".  \",\".. \",\"...\",\"   \"]},star:{fps:14,frames:[\"\\u2736\",\"\\u2738\",\"\\u2739\",\"\\u273A\",\"\\u2739\",\"\\u2737\"]},ball:{fps:8,frames:[\"\\u2801\",\"\\u2802\",\"\\u2804\",\"\\u2802\"]},triangle:{fps:15,frames:[\"\\u25E2\",\"\\u25E3\",\"\\u25E4\",\"\\u25E5\"]},circle:{fps:15,frames:[\"\\u25D0\",\"\\u25D3\",\"\\u25D1\",\"\\u25D2\"]},bounce:{fps:12.5,frames:[\"( \\u25CF    )\",\"(  \\u25CF   )\",\"(   \\u25CF  )\",\"(    \\u25CF )\",\"(     \\u25CF)\",\"(    \\u25CF )\",\"(   \\u25CF  )\",\"(  \\u25CF   )\",\"( \\u25CF    )\",\"(\\u25CF     )\"]},clock:{fps:10,frames:[\"\\uD83D\\uDD50 \",\"\\uD83D\\uDD51 \",\"\\uD83D\\uDD52 \",\"\\uD83D\\uDD53 \",\"\\uD83D\\uDD54 \",\"\\uD83D\\uDD55 \",\"\\uD83D\\uDD56 \",\"\\uD83D\\uDD57 \",\"\\uD83D\\uDD58 \",\"\\uD83D\\uDD59 \",\"\\uD83D\\uDD5A \"]},pong:{fps:12.5,frames:[\"\\u2590\\u2802       \\u258C\",\"\\u2590\\u2808       \\u258C\",\"\\u2590 \\u2802      \\u258C\",\"\\u2590 \\u2820      \\u258C\",\"\\u2590  \\u2840     \\u258C\",\"\\u2590  \\u2820     \\u258C\",\"\\u2590   \\u2802    \\u258C\",\"\\u2590   \\u2808    \\u258C\",\"\\u2590    \\u2802   \\u258C\",\"\\u2590    \\u2820   \\u258C\",\"\\u2590     \\u2840  \\u258C\",\"\\u2590     \\u2820  \\u258C\",\"\\u2590      \\u2802 \\u258C\",\"\\u2590      \\u2808 \\u258C\",\"\\u2590       \\u2802\\u258C\",\"\\u2590       \\u2820\\u258C\",\"\\u2590       \\u2840\\u258C\",\"\\u2590      \\u2820 \\u258C\",\"\\u2590      \\u2802 \\u258C\",\"\\u2590     \\u2808  \\u258C\",\"\\u2590     \\u2802  \\u258C\",\"\\u2590    \\u2820   \\u258C\",\"\\u2590    \\u2840   \\u258C\",\"\\u2590   \\u2820    \\u258C\",\"\\u2590   \\u2802    \\u258C\",\"\\u2590  \\u2808     \\u258C\",\"\\u2590  \\u2802     \\u258C\",\"\\u2590 \\u2820      \\u258C\",\"\\u2590 \\u2840      \\u258C\",\"\\u2590\\u2820       \\u258C\"]}});var cli$1={ArgParser:class{constructor(options){this.options=options||{}}parse(argv=process.argv){}},control,colorize,symbols,Spinner:class{constructor(name=\"circle\"){const spinner=spinners[name];this.frames=spinner.frames,this.index=0,this.interval=Math.round(1e3/spinner.fps),this.setIntervalIndex=null}next(){this.index++;const realIndex=(this.index-1)%this.frames.length;return this.frames[realIndex]}start(outputFunction){outputFunction=outputFunction||(str=>process.stdout.write(str)),this.setIntervalIndex=setInterval(()=>{let frame=this.next(),clearFrame=frame.replace(/./g,\"\\b\");outputFunction(clearFrame),outputFunction(frame)},this.interval)}stop(){clearInterval(this.setIntervalIndex)}}};const state={prompt:cli$1.colorize(\"gray\",\"jsh> \"),input:[],inputPosition:0,history:[],historyPosition:void 0};addEventListener(\"consoleInput\",({detail})=>handle(detail.key));const handle=key=>match(key)([[\"\\x7F\",\"\\b\"],remove],[[\"\\r\"],enter],[[\"\\x1B[A\",\"\\x1B[B\",\"\\x1B[C\",\"\\x1B[D\"],arrow])(insert),insert=key=>{state.input.splice(state.inputPosition,0,key),state.inputPosition++;const trailing=state.input.slice(state.inputPosition);print(key+cli$1.control.cursor.savePosition()+trailing.join(\"\")+cli$1.control.cursor.restorePosition())},remove=()=>{if(0!=state.inputPosition){state.inputPosition--,state.input.splice(state.inputPosition,1);const trailing=state.input.slice(state.inputPosition);print(cli$1.control.cursor.move.left()+cli$1.control.cursor.savePosition()+cli$1.control.line.eraseEnd()+trailing.join(\"\")+cli$1.control.cursor.restorePosition())}},enter=()=>{const result=evaluate(state.input.join(\"\"));JSON.stringify(state.input)!=JSON.stringify(state.history[0])&&state.history.unshift(state.input),state.historyPosition=void 0,state.input=[],state.inputPosition=0,print(\"\\n\"+result+\"\\n\"+state.prompt)},arrow=key=>\"\\x1B[A\"==key?historyBackward():\"\\x1B[B\"==key?historyForward():\"\\x1B[C\"==key?right():\"\\x1B[D\"==key?left():null,right=()=>{state.inputPosition==state.input.length||(state.inputPosition++,print(cli$1.control.cursor.move.right()))},left=()=>{0==state.inputPosition||(state.inputPosition--,print(cli$1.control.cursor.move.left()))},historyBackward=()=>{state.historyPosition!=state.history.length-1&&(void 0==state.historyPosition?state.input.length?(state.history.unshift(state.input),state.historyPosition=1):state.historyPosition=0:state.historyPosition++,state.input=state.history[state.historyPosition],print(cli$1.control.line.erase()+cli$1.control.cursor.move.leftMost()+state.prompt+state.input.join(\"\")))},historyForward=()=>{0==state.historyPosition||void 0==state.historyPosition||(state.historyPosition--,state.input=state.history[state.historyPosition],print(cli$1.control.line.erase()+cli$1.control.cursor.move.leftMost()+state.prompt+state.input.join(\"\")))};print(`Welcome to Faux's ${cli$1.colorize(\"bold\",\"J\")}avascript ${cli$1.colorize(\"bold\",\"SH\")}ell!`+\"\\n\\n\"+state.prompt)})();"
  })
);
// ls
root.addInode(
