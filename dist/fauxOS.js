...
(function (global, factory) {
    ...
    const init = new Process("(function(){\"use strict\";sys.exec(\"/bin/fsh\")})();");
    ...
    root.addInode(["bin"], "fsh", new Inode({
        file: true,
        executable: true,
        contents: "var __awaiter=this&&this.__awaiter||function(thisArg,_arguments,P,generator){return new(P||(P=Promise))(function(resolve,reject){function fulfilled(value){try{step(generator.next(value))}catch(e){reject(e)}}function rejected(value){try{step(generator[\"throw\"](value))}catch(e){reject(e)}}function step(result){result.done?resolve(result.value):new P(function(resolve){resolve(result.value)}).then(fulfilled,rejected)}step((generator=generator.apply(thisArg,_arguments||[])).next())})};(function(){\"use strict\";function prompt(str,color=\"gray\"){return __awaiter(this,void 0,void 0,function*(){const cwd=yield sys.pwd(),prompt=cli.colorize(color,str||`faux:${cwd} # `);return yield print(prompt)})}function tokenizeLine(line=\"\"){const tokens=line.match(/([\"'])(?:\\\\|.)+\\1|((?:[^\\\\\\s]|\\\\.)*)/g).filter(String);for(let token,i=0;i<tokens.length;i++)token=tokens[i],tokens[i]=token.replace(/\\\\(?=.)/g,\"\"),token.match(/^[\"'].+(\\1)$/m)&&(tokens[i]=/^([\"'])(.+)(\\1)$/gm.exec(token)[2]);return tokens}function lex(input=\"\"){const allTokens=[],lines=input.match(/(\\\\;|[^;])+/g);for(let tokens,i=0;i<lines.length;i++)tokens=tokenizeLine(lines[i]),allTokens.push(tokens);return allTokens}function parseCommand(tokens){return{type:\"simple\",argv:tokens,argc:tokens.length,name:tokens[0]}}function parse(input=\"\"){const AST={type:\"script\",commands:[]},commands=lex(input);for(let i in commands){const parsed=parseCommand(commands[i]);AST.commands[i]=parsed}return AST}function which(name){return __awaiter(this,void 0,void 0,function*(){const path=(yield sys.getenv(\"PATH\")).split(\":\");for(let i in path){const execPath=path[i]+\"/\"+name,pathStat=yield sys.stat(execPath);if(pathStat.file&&pathStat.executable)return execPath}})}function evaluate(str){return __awaiter(this,void 0,void 0,function*(){const ast=parse(str);for(let i in ast.commands){const command=ast.commands[i],execPath=yield which(command.name);sys.exec(execPath,command.argv)}return yield prompt()})}addEventListener(\"consoleInput\",function({detail}){return __awaiter(this,void 0,void 0,function*(){if(detail.buffered){const input=yield process.stdin.read();evaluate(input)}})}),prompt()})();"
    }));
    // Javascript SHell
