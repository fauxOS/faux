...
class Process {
  constructor(image, argv) {
    ...
    this.worker = utils.mkWorker(
      [/* syscalls */ "self.sys={newID(length=8){const chars=\"0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz\";let id=\"\";for(let i=0;i<length;i++){const randNum=Math.floor(Math.random()*chars.length);id+=chars.substring(randNum,randNum+1)}return id},call(name,args){const id=newID();return postMessage({type:\"syscall\",name:name,args:args,id:id}),new Promise(function(resolve,reject){self.addEventListener(\"message\",msg=>{msg.data.id===id&&(\"success\"===msg.data.status?resolve(msg.data.result):reject(msg.data.reason))})})},async load(path){const data=await call(\"load\",[path]);if(-2===data)return new Error(\"No data returned, possibly a directory\");return 0>data?new Error(\"Could not get data\"):self.eval(data)},spawn(image,argv=[]){return call(\"spawn\",[image,argv])},exec(path,argv){return call(\"exec\",[path,argv])},access(path){return call(\"access\",[path])},async open(path,mode=\"r\"){const fd=await call(\"open\",[path,mode]);return 0>fd?new Error(\"Could not open file\"):fd},async read(fd){const data=await call(\"read\",[fd]);if(-2===data)return new Error(\"No data returned, possibly a directory\");return 0>data?new Error(\"Could not get data\"):data},async write(fd,data){const ret=await call(\"write\",[fd,data]);return 0>ret?new Error(\"Could not write data\"):data},pwd(){return call(\"pwd\",[])},chdir(path){return call(\"chdir\",[path])},getenv(varName){return call(\"getenv\",[varName])},setenv(varName){return call(\"setenv\",[varName])}};" /* end */, lib, expose, image].join("\n\n")
    );
